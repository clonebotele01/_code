...
API_ID = 9579086     # See above for how to get it
#API_ID = 11393521
API_HASH = 'cbe2bba504ee95387a4d1ccef2f88a8e' # See above for how to get it
#API_HASH = '11aa0e24701cd0d4a711b7416c9185b5'
BOT_TOKEN = '5400470869:AAGyadNiiluhYbJldnbaYdTyv9vHTc8M0zI'
#BOT_TOKEN = '5572599694:AAELQJWOI2qHevUOPWxUTJbMSjmhVM68f4U'

my_channel_entity  = 1637929257
my_group_entity = 1001708791345
client = TelegramClient('anon_bot_QAT_220927_new', API_ID, API_HASH).start(bot_token = BOT_TOKEN)
...
async def cal_profit(_tokenCA,_initial):
    ...
    while profit > 50 and profit < 150 and trigger != True:
        profitCheck = await update_Token_Price(_tokenCA)
        profit = profitCheck[1] * 100 / _initial
        print("Current price: {}  - Wallet balance in BNB: {} - Profit: {}".format(profitCheck[2], profitCheck[1],
                                                                                   profit))
...
async def TokenBuy_BNB_with_TPSL(_tokenCA, _BNB_Amount, _GasWei, _GasLimit,_Slip,_numTx,_TP,_SL):
    ...
    ###
    asyncio.get_event_loop().create_task(cal_profit(_tokenCA,_BNB_Amount))
    # profitCheck = await update_Token_Price(_tokenCA)
    # profit = profitCheck[1] * 100 / _BNB_Amount
    # while profit > 50 and profit < 150 and trigger != True:
    #     profitCheck = await update_Token_Price(_tokenCA)
    #     profit = profitCheck[1] * 100 / _BNB_Amount
...
async def my_event_handler(event):
    ...
    if ( (msg_lower.split()[0] =="btpsl") and msg.split()[1].startswith("0x")) and len(msg.split()[1]) == 42 :
        token_flag = await checkTokenCA(msg.split()[1])
        if token_flag[0] == True:
            await sendMessage(
                "Buying Token: {} \n + BNB Amount: {} \n + Gaswei: {} \n + GasLimit: {}\n + Slippage: {} \n + Number of Tx: {}".format(token_flag[1],BNBAmount,Gaswei,GasLimit,Slip,numTx))
    ...
    if (msg_lower == ("triggeron")): trigger = True
