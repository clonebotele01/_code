...
async def cal_profit(_tokenCA,_initial):
    ...
    while profit > 50 and profit < 150 and trigger[_tokenCA] != True:
        profitCheck = await update_Token_Price(_tokenCA)
        profit = profitCheck[1] * 100 / _initial
...
async def TokenBuy_BNB_with_TPSL(_tokenCA, _BNB_Amount, _GasWei, _GasLimit,_Slip,_numTx,_TP,_SL):
    global trigger, token_track_tasks
    spend = web3.toChecksumAddress("0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c")  # WBNB Address
    tokenToBuy = web3.toChecksumAddress(_tokenCA)
    amountOut = pcs_contract.functions.getAmountsOut(web3.toWei(_BNB_Amount, 'ether'), [spend, tokenToBuy]).call()[-1]
    amountMin = amountOut / (1 + _Slip / 100)
    ...
    ###
    task = asyncio.get_event_loop().create_task(cal_profit(_tokenCA,_BNB_Amount))
    token_track_tasks.append(task)
    await asyncio.gather(*token_track_tasks)
