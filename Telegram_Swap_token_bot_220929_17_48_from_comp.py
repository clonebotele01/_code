...
if file_exists('Token_trade_log.json'):
    with open("./Token_trade_log.json") as f:
        trade_log_token = json.load(f)
else:
    tracking_file = open('Token_trade_log.json', "w+")
    tracking_file.write("{\n\n}")
    tracking_file.close()
...
async def update_Token_Price(_tokenCA):
    TokenContract = web3.eth.contract(address=web3.toChecksumAddress(_tokenCA),
                                      abi=ERC20abi_BSC)
    TokenInfo = await checkToken(checker, web3.toChecksumAddress(_tokenCA))
    token_decimal = TokenContract.functions.decimals().call()
    balance = TokenContract.functions.balanceOf(sender_wallet).call()
    readable = 0
    readable_in_usd = 0
    readable_in_bnb = 0
    readable_in_bnb_with_tax = 0

    BNB_price = await BNB_to_USDT()
    Tokenprice_in_BNB = int(pcs_contract.functions.getAmountsOut(1 * 10 ** token_decimal, [
        web3.toChecksumAddress(_tokenCA),
    ...
    if (balance != 0):
        ...
        readable_in_bnb_with_tax = readable_in_bnb - (readable_in_bnb * float(TokenInfo[1]) / 100)

    return readable_in_bnb, readable_in_bnb_with_tax, readable_in_usd
...
async def updateTradeLog(_trade_log_token) :
    tmp = json.dumps(_trade_log_token, indent=6)
    f = open('Token_trade_log.json', 'w')
    print(tmp, file=f)
    f.close()

async def TokenBuy_BNB(_tokenCA, _BNB_Amount, _GasWei, _GasLimit,_Slip,_numTx):
    global trade_log_token
    TokenToSell = web3.toChecksumAddress(_tokenCA)
    sellTokenContract = web3.eth.contract(TokenToSell, abi=ERC20abi_BSC)
    token_decimal = sellTokenContract.functions.decimals().call()
    balance_token_inital = sellTokenContract.functions.balanceOf(sender_wallet).call()
    ...
    balance_bnb_inital = web3.fromWei(web3.eth.get_balance(sender_wallet), 'ether')
    count = 0
    ...
    balance_bnb_final = web3.fromWei(web3.eth.get_balance(sender_wallet), 'ether')
    gas_fee = float(balance_bnb_inital) - float(balance_bnb_final) - float(_BNB_Amount)

    if web3.eth.get_transaction_receipt(tx_token).status == 1:
        await sendMessage("Transaction status: SUCCESS - Gas fee: {:.3f}BNB!!! \n".format(gas_fee))
    else:
        ...
        try:
            ...
            await sendMessage("Transaction status: FAILED - Gas fee: {:.3f}BNB!!! \n".format(gas_fee))
    ...
    ## Log collecting ##
    balance_token_final = sellTokenContract.functions.balanceOf(sender_wallet).call()
    if _tokenCA in trade_log_token:
        print("In")
        prv_bnb_buy = trade_log_token[_tokenCA]["BNB Buy"]
        prv_bnb_profit = trade_log_token[_tokenCA]["Profit"]
        trade_log_token[_tokenCA]["BNB Buy"] = float(prv_bnb_buy) + float(balance_bnb_inital) - float(balance_bnb_final)
        trade_log_token[_tokenCA]["Profit"] = trade_log_token[_tokenCA]["BNB Sell"] - trade_log_token[_tokenCA]["BNB Buy"]
    else:
        print("Not in")
        trade_log_token[_tokenCA] = dict()
        trade_log_token[_tokenCA]["CA"] = _tokenCA
        trade_log_token[_tokenCA]["BNB Buy"] = float(balance_bnb_inital) - float(balance_bnb_final)
        trade_log_token[_tokenCA]["BNB Sell"] = 0
        trade_log_token[_tokenCA]["Profit"] = trade_log_token[_tokenCA]["BNB Sell"] - trade_log_token[_tokenCA]["BNB Buy"]

    await updateTradeLog(trade_log_token)

async def cal_profit(_tokenCA,_initial):
    global trigger, TL, SL
    trigger[_tokenCA] = False
    profitCheck = await update_Token_Price(_tokenCA)
    profit = float(profitCheck[1]) * 100 / float(_initial)
    while profit > SL and profit < TP and trigger[_tokenCA] != True:
        profitCheck = await update_Token_Price(_tokenCA)
        profit = profitCheck[1] * 100 / _initial
        #print("Current price: {}  - Wallet balance in BNB: {} - Profit: {}".format(profitCheck[2], profitCheck[1],profit))
        await asyncio.sleep(1)

    if trigger[_tokenCA] == True :
        await sendMessage("Stop tracking price of  token: {}".format(_tokenCA))
    else:
        await sendMessage("Profit reached: {}% --> Now selling!".format(profit))
        await sendMessage("s {}".format(_tokenCA))

async def TokenBuy_BNB_with_TPSL(_tokenCA, _BNB_Amount, _GasWei, _GasLimit,_Slip,_numTx,_TP,_SL):
    global trigger, token_track_tasks, task
    ...
    try :
        ...
        task[_tokenCA] = asyncio.get_event_loop().create_task(cal_profit(_tokenCA, _BNB_Amount))
    else:
        task[_tokenCA].cancel()
        task[_tokenCA] = asyncio.get_event_loop().create_task(cal_profit(_tokenCA, _BNB_Amount))
...
async def TokenSell_BNB(_web3, _tokenCA, _Token_Percent, _GasWei, _GasLimit, _Slip):
    ...
    balance_bnb_inital = _web3.fromWei(web3.eth.get_balance(sender_wallet), 'ether')
    ...
    if allowance > tokenValue:
        ...
        if web3.eth.get_transaction_receipt(tx_token).status == 1:
            gas_fee = web3.eth.waitForTransactionReceipt(_web3.toHex(tx_token)).gasUsed * web3.eth.getTransaction(_web3.toHex(tx_token)).gasPrice / 10 ** 18
            await sendMessage("Transaction status: SUCCESS !!! \nTX Hash: https://bscscan.com/tx/{} \nGas fee: {:.3f}".format(_web3.toHex(tx_token),gas_fee))
        else:
            tx = web3.eth.get_transaction(tx_token)
            replay_tx = {
                'to': tx['to'],
            ...
            try:
                ...
                gas_fee = web3.eth.waitForTransactionReceipt(_web3.toHex(tx_token)).gasUsed * web3.eth.getTransaction(_web3.toHex(tx_token)).gasPrice / 10 ** 18
                await sendMessage("Transaction status: FAILED !!! \nTX Hash: https://bscscan.com/tx/{} \nFail reason: {} !!! \nGas fee: {:.3f}".format(_web3.toHex(tx_token),e,gas_fee))
                if "INSUFFICIENT_OUTPUT_AMOUNT" in str(e):
                    await sendMessage("Maybe slippage is to small, please increase your Slippage setting !!! \n")

            ## Log collecting ##
            balance_bnb_final = web3.fromWei(web3.eth.get_balance(sender_wallet), 'ether')
            balance_token_inital = balance
            balance_token_final = sellTokenContract.functions.balanceOf(sender_wallet).call()
            if _tokenCA in trade_log_token:
                prv_bnb_sell = trade_log_token[_tokenCA]["BNB Sell"]
                prv_bnb_profit = trade_log_token[_tokenCA]["Profit"]
                trade_log_token[_tokenCA]["BNB Sell"] = float(prv_bnb_sell) + float(balance_bnb_final) - float(balance_bnb_inital)
                trade_log_token[_tokenCA]["Profit"] = trade_log_token[_tokenCA]["BNB Sell"] - trade_log_token[_tokenCA]["BNB Buy"]
            else:
                trade_log_token[_tokenCA] = dict()
                trade_log_token[_tokenCA]["CA"] = _tokenCA
                trade_log_token[_tokenCA]["BNB Buy"] = 0
                trade_log_token[_tokenCA]["BNB Sell"] = float(balance_bnb_final) - float(balance_bnb_inital)
                trade_log_token[_tokenCA]["Profit"] = trade_log_token[_tokenCA]["BNB Sell"] - trade_log_token[_tokenCA]["BNB Buy"]
    else:
...
async def my_event_handler(event):
    ...
    global track_list_dict,trigger
            ...
            if (token.startswith("0x") and len(token) == 42):
                ...
                if _checkToken[0] == True :
                    if token not in track_list_dict:
                        ...
                        await sendMessage("Added token {} to track list!".format(_checkToken[1]))
                    else :
    ...
    if ( (msg_lower.split()[0] =="b1") and msg.split()[1].startswith("0x")) and len(msg.split()[1]) == 42 :
        token_flag = await checkTokenCA(msg.split()[1])
        if token_flag[0] == True:
            await sendMessage(
                "Buying Token: {} \n + BNB Amount: {} \n + Gaswei: {} \n + GasLimit: {}\n + Slippage: {} \n + Number of Tx: {}".format(token_flag[1],BNBAmount,Gaswei,GasLimit,Slip,numTx))
            await TokenBuy_BNB_with_TPSL(msg.split()[1], BNBAmount, Gaswei, GasLimit,Slip,numTx,TP,SL)
    ...
    if (msg_lower.split()[0] == "trigger" and msg.split()[1].startswith("0x")) and len(msg.split()[1]) == 42: trigger[msg.split()[1]] = True
